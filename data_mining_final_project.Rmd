...
Manhattan Distance can be thought of as the shortest path between two points in euclidean space, with the restraint that the path can only move at right angles (similiar to a taxi car traversing a city). Manhattan Distance could be useful where the data naturally expresses distance in this format, for example, a chessboard can only be traversed in this form. The green line in the image below represents euclidean distance, and the red, blue, and yellow lines represent the shortest paths using Manhattan distance. Image from [wikipediea](https://en.wikipedia.org/wiki/Taxicab_geometry)
...
Although the iterative algorithm is fairly simple, the quest remains on how to initialize the K means. There are two commonly used methods: the Forgy method and the Random Partition.
...
The `kmeans` function takes in the data, the number of centers (K), and the number of starts: this is the number of times that K-Means is run, choosing the 'best' run based on the smallest within-cluster sum of squares. The resulting variables contains a vector of the cluster assignment to each point; we can plot these visually below:
...
- Guarantee of Convergence (4)
  - Although no guarantee that converged to optimal solution
- Scalability
...
One of the disadvantages discussed above in K-Means is that points are assigned a hard classification to a single. However, we can utilize distribution-based clustering to receive a soft-classification, or a probability, of a point belonging to a cluser. One of the main Distribution-Based Clustering algorithms is Gaussian Mixture Models. Gaussian Mixture Models treat the probability density disribution as being constructed of K different distributions:
...
2. The *responsibilities* of each data point $r_{ij}$
    - This represents the posterior probability that each data point $i$ came from each distribution $k$.
    
These values are estimated using the iterative EM algorithm. This algorithm consists of the following steps steps:
...
As mentioned above the responsibilities calculated during the EM algorithm are *probabilities*: this provides more information than the hard classification provided by K-Means because it provides us a measure of uncertainty in our results. This is especially important at the boundary points between the two distributions, allowing to see how confident we can be in our results.
...
- Statistical View of Clustering (6)
  - Allows us to view the clustering problem as attempting to find the underlying distributions that generate the data
- Easily Interpretable
  - Mixture Models are easy to understand as the weighted sum of different density distributions, and these distributions can be more easily visually represented than K-Means
...
https://www.datanovia.com/en/lessons/clustering-distance-measures/#methods-for-measuring-distances
...
https://mdporter.github.io/SYS6018/lectures/06-clustering.pdf
